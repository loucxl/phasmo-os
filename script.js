// --- 1. CORE DATA ---
const EVIDENCE = [
    { id: 'emf', label: 'EMF 5', icon: 'ðŸ“¶', desc: "Must hit Level 5 (Red). Each ghost interaction has a 33% chance to produce EMF 5 if the ghost has this evidence." },
    { id: 'box', label: 'Box', icon: 'ðŸ“»', desc: "Lights OFF in the ghost's room. Must be in the same room (or nearby for ghosts that respond to everyone). Deogen responds in any lighting." },
    { id: 'uv', label: 'UV', icon: 'ðŸ–ï¸', desc: "Green handprints." },
    { id: 'orb', label: 'Orbs', icon: 'âœ¨', desc: "Floating dots in NV." },
    { id: 'writing', label: 'Writing', icon: 'ðŸ“–', desc: "Ghost writes in book." },
    { id: 'freezing', label: 'Freeze', icon: 'â„ï¸', desc: "Below 0Â°C on thermometer. Note: visible breath appears below 5Â°C for all ghosts â€” breath alone does NOT confirm freezing evidence." },
    { id: 'dots', label: 'D.O.T.S', icon: 'ðŸŸ¢', desc: "Green silhouette." }
];

const FILTERS = [
    { id: 'fast', label: 'âš¡ Speed Change' },
    { id: 'early', label: 'âš ï¸ Early Hunter' },
    { id: 'quiet', label: 'ðŸ¤« Quiet Footsteps (Myling)' },
    { id: 'guarantee', label: 'âœ¨ Guaranteed Ev' }
];
...
const GHOSTS = [
    ...
    { 
        ...
        zeroEv: "Watch for random EMF readings appearing right next to a player in a room far from the active area â€” that's a Wraith teleport. In 0EV, use salt as a physical test: Wraith should not disturb it at all.",
        tags: ['guarantee'],
        speedStates: [{"label": "Normal", "speed": 1.7}]
    },
    { 
    ...
    { 
        ...
        test: "Pile Test: Create a large pile of throwable objects in the active area. A Poltergeist can 'explode' the entire pile, scattering items in all directions simultaneously. Also note that Poltergeists throw objects <span class='hl-green'>every 0.5 seconds</span> during hunts (100% chance each interval) vs other ghosts' 50% chance â€” much more frequent mid-hunt throwing.",
        zeroEv: "Constant, heavy object throwing â€” way more than any other ghost. Objects thrown during hunts very frequently. Each thrown object drains 2% sanity from nearby players, so rapid sanity loss in a cluttered room is suspicious. Exploding a pile of objects is a near-certain tell.",
        tags: ['vis'],
        speedStates: [{"label": "Normal", "speed": 1.7}]
    },
    { 
        ...
        test: "Parabolic Mic Test: Point the parabolic mic toward the active area from outside. If you hear a <span class='hl-green'>unique screech/wail sound</span> that other ghosts don't make, it's a Banshee. Important: <strong>any player</strong> can hear this scream â€” not just the target. In multiplayer, watch who the ghost chases during hunts â€” it will always ignore other players and beeline for its target, even walking past them.",
        zeroEv: "In multiplayer: one player always gets chased regardless of others being nearby, and the ghost ignores everyone else mid-hunt. The Banshee hunts using its TARGET's sanity only (not team average), so it can hunt early if one player has low sanity even while others are high. Unique screech on parabolic mic is audible to any player, not just the target â€” a reliable solo and team tell.",
        tags: ['guarantee'],
        speedStates: [{"label": "Normal", "speed": 1.7}]
    },
    { 
    ...
    { 
        ...
        desc: "Empowered by darkness, weakened by light. Hunts earlier in dark rooms (60%) and less often in lit rooms (40%). Will immediately switch lights OFF and frequently breaks bulbs. Does not actively seek dark rooms â€” it roams more when its current room is lit, which tends to move it around frequently.",
        ability: "Hunts at 60% sanity in DARK rooms but only 40% sanity in LIT rooms. Will immediately turn lights OFF (within seconds) and frequently shatters bulbs. Can NEVER turn lights ON â€” only off. Note: despite patch notes suggesting otherwise, the Mare currently cannot turn TVs or computers on either (a known in-game bug).",
        test: "Light Test: Switch ON a light in the active area. A Mare can <span class='hl-red'>immediately</span> turn a light back off after you switch it on (within 4m, ~10s cooldown per light) â€” repeated instant shut-offs are a strong tell. It cannot turn lights ON, only off. You can also test hunt threshold: if it hunts in a lit room, sanity must be below 40%. In a dark room, it can hunt at 60%. Keeping the active area lit forces it to a lower hunt threshold.",
        zeroEv: "Lights constantly being switched off, never turned on. Frequent light bulb breaking events. Hunts earlier in dark areas (60%) but safer if room is lit (40% threshold). If a light goes off almost immediately after you turn it on, that's a strong Mare indicator. Note: the Mare does not seek out dark rooms â€” it simply roams more when its current room is lit, which tends to move it around the map frequently.",
        tags: ['early'],
        speedStates: [{"label": "Normal", "speed": 1.7}]
    },
    { 
        ...
        test: "Speed Listen Test: During a hunt, listen carefully when hidden. Revenant footsteps should be <span class='hl-green'>very slow (1 m/s)</span> when it hasn't seen anyone â€” slower than a walking player. If it suddenly spots someone, it rockets to 3 m/s (nearly sprint speed). This dramatic slow-to-fast switch is unmistakable. Break line of sight by ducking through a doorway to make it slow again.",
        zeroEv: "The Revenant's slow-to-fast speed pattern is its biggest tell: methodically slow footsteps while searching (1 m/s), then extremely rapid footsteps when it detects a player (3 m/s). If you hear it speed up without being chased, it may have detected voice or equipment. Never run in a straight line â€” always corner or break LoS.",
        tags: ['fast'],
    ...
    { 
        ...
        desc: "The shyest ghost. Very low activity. Cannot initiate a hunt OR produce an EMF reading while any player is in the same room as it â€” this checks the Shadeâ€™s current room, not necessarily its favourite room. Groups are safer, but note it can still reach into an occupied room to interact with objects from an adjacent room.",
        ability: "Cannot initiate a hunt if any player is in the same room as it â€” even a single player prevents it from hunting. Hunts at only 35% sanity (lower than normal 50%). Generally very passive with low activity levels.",
        test: "Group Test: Stay in the active area as a team with a crucifix placed. Let sanity drop below 35%. If the ghost refuses to hunt while any player is in the room despite very low sanity, it's very likely a Shade. You can also try placing a Ghost Writing book alone â€” the Shade won't write in it if a player is in the room. Send one person in solo to trigger activity.",
        zeroEv: "Extremely passive â€” very low ghost activity, rare interactions, barely any events. Will not hunt OR produce EMF readings while any player is in the same room (not just the favourite room). It can still reach into the room from an adjacent one to interact with objects. If activity is near-zero while players are present but picks up when the room is empty, suspect Shade.",
        tags: [],
        speedStates: [{"label": "Normal", "speed": 1.7}]
    },
    { 
        ...
        test: "Smudge Timing Test: Smudge the ghost when it's NOT hunting and start a timer. If it initiates a hunt <span class='hl-red'>between 60â€“90 seconds</span> after being smudged (vs Spirit's 180s or standard 90s), it's likely a Demon. Also watch for hunts at high sanity (70%+) and very short gaps between consecutive hunts (20s cooldown vs 25s normally).",
        zeroEv: "Frequent hunts starting at 60â€“70% sanity â€” you'll be hunted way earlier than expected. Very short cooldown between hunts. Smudge blocks hunting for only 60 seconds. Crucifix range is 50% larger against Demon: T1 4.5m, T2 6m, T3 7.5m. Rare chance to hunt at ANY sanity, even 100%.",
        tags: ['early'],
        speedStates: [{"label": "Normal", "speed": 1.7}]
    },
    { 
        ...
        ability: "Can smoothly close a door fully (without creaking) and drain 15% sanity from nearby players â€” this is its unique ability. Only ghost that can close EXIT doors outside of hunts. When smudged, trapped in its room for ~90 seconds. Bug: can partially open locker doors.",
        test: "Door Slam Test: Leave doors in the active area propped open at 45Â°. A Yurei can <span class='hl-green'>fully close a door in one smooth motion</span> (no creaking) while draining 15% sanity from nearby players â€” this is its unique ability. Critically, if a door leading OUTSIDE the building fully closes without a hunt or event, it's <span class='hl-green'>100% a Yurei</span> (only ghost that can do this). Smudge it and place motion sensors at doorways â€” if it doesn't leave for 90s, supports Yurei.",
        zeroEv: "A door closing fully and smoothly (not slamming during a hunt/event) with a sudden 15% sanity drop is the clearest tell. Watch exit/building doors especially â€” only Yurei closes those outside of hunts. After smudging, the Yurei gets temporarily confined to its room (use motion sensors to verify it hasn't left).",
        tags: [],
        speedStates: [{"label": "Normal", "speed": 1.7}]
    },
    { 
        ...
        ability: "Cannot perform the 'airball' mist event (floating ball of mist). Drains 20% sanity during WALKING manifestation event collisions (vs 10% normal). Note: singing events only drain 10% due to a known bug. More visible during hunts â€” stays visible longer between flickers.",
        test: "Airball Elimination Test: Watch ghost events carefully. If you visually see the 'airball' event â€” a small ball of mist/smoke floating towards you â€” it is <span class='hl-red'>100% NOT an Oni</span>. Important: a walking ghost manifestation can also produce the same hiss sound as an airball event. You must actually <strong>see</strong> the floating mist ball, not just hear the hiss, before ruling out Oni. During hunts, the Oni stays visible longer between flickers â€” more visible than other ghosts. Very high object interaction rate is also a strong indicator.",
        zeroEv: "Very high activity level â€” lots of physical interactions and events. Cannot be an Oni if you visually see the airball mist event (the hiss sound alone is not enough â€” walking manifestation events make the same hiss). During hunts, the ghost stays visible longer than normal. Sanity drain: Oni drains 20% on WALKING manifestation collisions, but only 10% on SINGING events (a known in-game bug).",
        tags: ['vis'],
        speedStates: [{"label": "Normal", "speed": 1.7}]
    },
    { 
        ...
        zeroEv: "Hunts at 80% sanity if players are talking within 2m of the ghost â€” stop talking near the active area! During hunts, Yokai can only hear voices and detect electronics within 2.5m (vs 12m normally), making it very easy to hide from if you stay quiet and keep distance. Both early hunts AND 'deaf' hunting behavior together strongly suggest Yokai.",
        tags: ['early'],
        speedStates: [{"label": "Normal", "speed": 1.7}]
    },
    { 
        ...
        ability: "Speed scales with temperature: 1.4 m/s in warm â†’ 2.7 m/s in freezing. No LOS speed-up â€” Hantu moves at a fixed speed based on room temperature only. Shows visible frosty breath during hunts even in warm areas. Will never turn breaker ON (but can turn it OFF). Has double the normal chance to turn breaker off.",
        test: "Temperature Speed Test: Keep the breaker ON to warm rooms. During a hunt in a warm area, a Hantu slows to 1.4 m/s. In a freezing room it hits 2.7 m/s. Look for <span class='hl-green'>visible frosty breath clouds</span> emanating from the ghost during hunts â€” this is unique to Hantu and appears even in warm rooms. Hantu also has a doubled chance to turn the breaker OFF, but will NEVER turn it ON.",
        zeroEv: "Visible freezing breath clouds from the ghost during a hunt, even in warm areas â€” this is Hantu's most reliable visual tell. Speed varies dramatically between rooms: very fast in cold rooms, noticeably sluggish in warm ones. The breaker frequently goes off, but never gets turned back on by the ghost.",
        tags: ['fast'],
    ...
    { 
        ...
        test: "Camera-Only D.O.T.S Test (Nightmare/Insanity only): Set up a video camera pointing at the D.O.T.S projector and leave the room. If you see the D.O.T.S silhouette <span class='hl-green'>ONLY through the camera feed</span> and NEVER with your naked eyes, it's a Goryo. The D.O.T.S also only appears when NO players are in the room. <span class='hl-red'>IMPORTANT: On 0-evidence custom difficulty, Goryo shows NO D.O.T.S at all</span> â€” the only tell is that it never changes favourite room and only performs short roams.",
        zeroEv: "<span class='hl-red'>On 0 evidence: Goryo is nearly impossible to confirm.</span> D.O.T.S does not appear at all. Your only tells are behavioural: it will never change its active area, only performs short roams (use salt/motion sensors to observe), and has the highest interaction rate of any ghost. On Nightmare/Insanity (1â€“2 ev), if D.O.T.S is one of the shown evidences, it will ONLY appear via video camera â€” never to the naked eye.",
        tags: ['guarantee'],
        speedStates: [{"label": "Normal", "speed": 1.7}]
    },
    { 
        ...
        desc: "Has very short-range footsteps and vocals during hunts â€” only audible within 12m instead of the normal 20m. Can still make vocal sounds during hunts, they're just quieter at the same distance. Very vocal on parabolic mic outside of hunts.",
    ...
    { 
        ...
        desc: "Actually a single ghost with two interaction ranges. Can interact in two places at once. When hunting, one twin is slower (1.53 m/s) and the other faster (1.87 m/s) â€” they alternate who hunts.",
        ability: "Two ghosts: Main entity (1.53 m/s, âˆ’10%) and Decoy entity (1.87 m/s, +10%). They alternate which one initiates hunts. Can interact with the environment in two different locations simultaneously.",
        test: "Dual Interaction Test: Watch for interactions happening in <span class='hl-green'>two different rooms simultaneously</span> â€” this is only possible with the Twins. You may see EMF readings or object interactions in different locations at once. During hunts, listen for speed variation across different hunts: one hunt may feel slower (~1.53 m/s) and the next faster (~1.87 m/s) â€” this is the two twins alternating who initiates.",
        zeroEv: "Simultaneous interactions in separate rooms are the clearest tell â€” e.g. a door in one room and an object in another moving at the same time. Hunt speeds will feel inconsistent across hunts (one is 10% slower, one is 10% faster than normal 1.7 m/s). The ghost may also start a hunt from an unexpected location far from the active area.",
        tags: ['fast'],
    ...
    { 
        ...
        ability: "Near active electronics, moves at 2.5 m/s and can hunt at 65% sanity instead of 50%. Disrupts electronics from 15m away (vs normal 10m). Note: only held/carried electronics trigger the early hunt threshold â€” placed DOTS T2/T3, motion sensors, and sound sensors do not count.",
        test: "Electronics Speed Test: Leave active equipment (flashlights, D.O.T.S, EMF readers) on the floor in/near the active area. During a hunt, if the ghost moves at <span class='hl-green'>2.5 m/s near your gear</span> and slows to 1.7 m/s away from it, it's a Raiju. Also: you'll hear your heartbeat from 15m away (vs 10m for other ghosts) and electronics flicker from further away (15m vs 10m). Note: Most active electronics count. Exceptions include motion sensors, sound sensors, and DOTS projectors when thrown/held; donâ€™t rely on those for a Raiju test.",
        zeroEv: "Very fast movement during hunts specifically near active electronics â€” slows noticeably when away from gear. Electronics flickering from unusually long range (15m). Can hunt at 65% sanity if active equipment is nearby. To slow it down, turn off flashlights and pick up any active equipment during a hunt.",
        tags: ['fast', 'early'],
    ...
    { 
        ...
        test: "Fingerprint Test: Check all doors and surfaces the ghost touches for UV fingerprints. Obake has a <span class='hl-red'>75% chance to leave fingerprints</span> (not 100% like other ghosts), and those prints fade in ~30s instead of 60s. If you find a <span class='hl-green'>6-finger handprint</span> on a door, it's 100% confirmed Obake. During hunts, watch for the ghost's model briefly changing to a different ghost â€” this happens at least once per hunt.",
    ...
    { 
        ...
        desc: "Getting a Spirit Box response CURSES you, doubling your sanity drain! Moves faster as sanity drops (1.5 â†’ 2.25 m/s). Sanity pills slow it down!",
        ability: "Spirit Box response or parabolic microphone sounds curse the player, causing 2x passive sanity drain (lights won't stop it). Speed increases from 1.5 m/s at high sanity to 2.25 m/s at low sanity (up to 3.71 m/s with LOS at 0% sanity). Smudge blind during hunts lasts ~7 seconds (vs ~5s for other ghosts â€” a known bug vs intended 7.5s).",
        test: "Curse & Pill Test: Get a Spirit Box response (this curses the respondent). If that player's sanity drops even while standing in lit areas â€” which normally stops drain â€” they're cursed by a Moroi. <span class='hl-green'>Taking Sanity Pills removes the curse</span>. During a hunt, smudge it and count: Moroi is blinded for ~7 seconds (vs 5s for other ghosts). Also watch ghost speed across multiple hunts â€” it gets progressively faster as average sanity drops.",
        zeroEv: "Gets noticeably faster in later hunts as team sanity drops â€” can become one of the fastest ghosts in the game near 0% sanity (up to ~3.71 m/s). Spirit Box is forced evidence on Nightmare. If a cursed player's sanity drains in the light and pills fix it, that strongly points to Moroi.",
        tags: ['fast'],
    ...
    { 
        ...
        test: "Hiding Test: Try to hide in a closet or locker. <span class='hl-red'>Deogen always knows where you are</span> â€” it will walk directly to your hiding spot every single time. Your only survival option is to loop it around furniture, as it slows to 0.4 m/s when within ~2.5m of a player. Spirit Box is forced evidence on Nightmare, and Deogen gives a <span class='hl-green'>unique heavy breathing/bull-like response</span> that sounds different from normal spirit box answers.",
        zeroEv: "Ghost immediately walking to exactly where every player is hiding â€” no hesitation, never searches. Very fast from distance (3 m/s) then dramatically slows to a crawl up close (0.4 m/s). Spirit Box gives a unique heavy breathing sound. Loop it around furniture to survive â€” never hide in enclosed spots.",
        tags: ['fast', 'guarantee'],
        speedStates: [{"label": "From distance", "speed": 3.0}, {"label": "Up close", "speed": 0.4}]
    },
    { 
        name: "Thaye", ev: ['orb','writing','dots'], danger: "High â†’ Low", hunt: "75% â†’ 15%", speed: "2.75 â†’ 1.0 m/s", blink: "Normal", forced: null,
        ...
        test: "Age Progression Test: Thaye starts young (75% hunt threshold, 2.75 m/s) and ages every 1â€“2 minutes <span class='hl-green'>only while players are nearby</span>. If the ghost was terrifyingly aggressive at the start but noticeably slowed and became passive over time, it's a Thaye. You can also ask its age on the Ouija Board â€” it will answer with a number. Older = slower and less active.",
        zeroEv: "Very high activity and aggression early on â€” hunts at 75% sanity and moves faster than almost any ghost. Activity and speed wind down progressively as time passes with players nearby. Late-game it may barely hunt at all (15% threshold, 1 m/s). Note: if no players are near the active area, it won't age â€” it stays young and dangerous.",
        tags: ['fast', 'early'],
    ...
    { 
        ...
        test: "Stand Still Test: During a hunt, when the ghost is within 10m, <span class='hl-green'>stop moving completely</span>. Dayan slows dramatically to 1.2 m/s when players stand still, and speeds back up to 2.25 m/s the instant you move. This speed swing is very noticeable. Also: if hunts start early while players are moving nearby (65% threshold while moving) but stop when you freeze, that's a strong tell. Dayan is always a female ghost model â€” check the name/gender in the journal.",
        zeroEv: "The most reliable test: freeze completely during a hunt. If the ghost abruptly slows to barely moving and then rockets forward again the instant you step, it's Dayan. Early hunts are more likely when players are moving near her, safer when standing still (45% vs 65% threshold). Always female gender â€” easy to check in journal.",
        tags: ['fast'],
    ...
    { 
        ...
        test: "Provocation Test: Use a crucifix or smudge sticks deliberately. A Gallu enters an <span class='hl-red'>Enraged state</span> (60% threshold, 1.96 m/s, only 4s incense blind) when protective gear is used â€” or when it steps in salt. After the enraged hunt ends it drops to a <span class='hl-green'>Weakened state</span> (40% threshold, 1.36 m/s, 6s incense blind). Key tell: in Enraged state the Gallu does NOT disturb salt. If it walks through a pile and leaves no impression, it is Enraged.",
        zeroEv: "Ghost becoming more aggressive after you use defensive equipment. Three-phase cycle: Normal â†’ Enraged (after salt/smudge/crucifix) â†’ Weakened (after enraged hunt ends). Key tells: Enraged = won't disturb salt + 4s incense blind; Weakened = disturbs salt + 6s incense blind. Multiplayer note: only the HOST reliably sees Gallu disturb salt â€” non-host players may see it walk through without disturbing it.",
        tags: ['early', 'fast'],
    ...
    { 
        ...
        ability: "Always starts CALM, first switches 1 minute after an exit door is first opened, then flips every 2 minutes (it can even switch mid-hunt). Calm state: 10% hunt threshold, 1.45 m/s, and counter-intuitively <span class='hl-green'>very HIGH activity</span> â€” lots of interactions, easy to track. Aggressive state: 65% threshold, 1.96 m/s, but <span class='hl-red'>LOW ambient activity</span> â€” it goes quiet right when it's most dangerous. Hunts that start while aggressive are 20% shorter than normal.",
        test: "Phase Watch Test: Track the clock from when the front door first opens â€” 1 minute to the first flip, then every 2 minutes. An Obambo alternates between a <span class='hl-green'>Calm phase</span> (interaction-heavy, easy evidence, 10% threshold, 1.45 m/s) and an <span class='hl-red'>Aggressive phase</span> (eerily quiet but hunting at 65%, 1.96 m/s). Do NOT use 'it's quiet so we're safe' logic â€” quiet is the dangerous phase. A mid-hunt speed snap (slow to fast or back, for no visible reason) is a strong confirm, as is timing a fast-start hunt that ends ~20% early.",
        zeroEv: "Behaviour that cycles in waves on a repeating ~2-minute timer: an activity-heavy phase where it barely hunts, then a quiet phase where hunts start at surprisingly high sanity. Its speeds can be mistaken for The Twins (similar 1.45/1.96 split) â€” but if the ghost consistently starts hunts at the FAST speed while sanity is above ~50%, it's far more likely Obambo (Twins alternate randomly). A fast-start hunt that ends noticeably early (20% shorter) or a mid-hunt speed change with no cause is near-conclusive.",
        tags: ['fast', 'early'],
        speedStates: [{"label": "Calm phase", "speed": 1.45}, {"label": "Aggressive phase", "speed": 1.96}]
    }
        ,
    { 
        name: "Aswang", ev: ['freezing','writing','dots'], danger: "Med", hunt: "50%", speed: "1.53 â†’ 2.53 m/s", blink: "Normal", forced: null,
        traits: ["Faster LoS Acceleration", "Cannot Kill Hidden Players", "Chase-Focused"],
        desc: "A chase-focused predator with the fastest line-of-sight acceleration in the game â€” but it physically cannot kill a player who is correctly hidden in an official hiding spot. Reaching one ends the hunt instantly.",
        ability: "Base hunt speed of 1.53 m/s (slightly slower than normal), ramping to a max LoS speed of ~2.53 m/s. Its acceleration rate is 1.5x the standard (0.075x base/s vs 0.05x), so it hits max speed in about <span class='hl-red'>8.7 seconds</span> of continuous line-of-sight instead of the usual ~13. Signature weakness: if it detects and reaches a player correctly hidden inside an OFFICIAL hiding spot (closet, locker, blockable spot), the hunt <span class='hl-green'>ends instantly</span> â€” it cannot kill hidden players. Caution: if a hunt ends this way, it starts its NEXT hunt by walking straight to that player's last hiding location.",
        test: "Hiding Spot Test: During a hunt, get its attention (incense helps) and duck into an official hiding spot like a closet or locker while it can see you. If the hunt <span class='hl-green'>ends the moment it reaches you</span>, it's 100% an Aswang â€” no other ghost does this. Unofficial spots (behind doors, dark corners) do NOT trigger this. Secondary tell: it builds to max chase speed noticeably faster than any other ghost (~8.7s of line-of-sight vs ~13s standard), so don't loop it for long.",
        zeroEv: "The hiding spot test works with zero evidence and is conclusive: a hunt that cancels itself the instant the ghost reaches your closet/locker is an Aswang. Speed profile: starts slightly slow (1.53 m/s, same base as Twins/Obambo) but accelerates on sight much faster than normal. Note: it CAN still kill you in unofficial hiding spots, and its weakness is disabled if custom difficulty sets hiding places to 0. After a cancelled hunt, expect it to check your old hiding spot first next hunt â€” hide somewhere new.",
        tags: ['fast'],
    ...
    { 
        ...
        desc: "A blind ghost that hunts entirely by sound. It cannot see still, silent players â€” but it kills anyone within 1.5m on the same floor, even through furniture and walls, so standing in its path is fatal.",
        ability: "The Kormos is blind: it can only gain line-of-sight if a player is within 5m with nothing in the way. Instead it hears footsteps â€” crouch-walking is detectable from 10m, walking from 15m, sprinting from 30m (same floor only) â€” plus voices and active electronics at normal ranges. When it hears something it moves to that sound at <span class='hl-red'>2.21 m/s</span> (1.3x its 1.7 m/s base) and can re-target to newer sounds every 5â€“10 seconds. Danger: with no line-of-sight it kills any player within <span class='hl-red'>1.5m on the same floor, ignoring walls and furniture</span>. Sprinting in the same room as it raises its hunt threshold from 50% to 70%. It cannot perform the mist-orb or chasing ghost events.",
        test: "Stand-Still Test: During a hunt, in an open area with an escape route, go completely still and silent with electronics off. A Kormos will walk straight past a player any other ghost would chase on sight. Confirm it's not a Banshee ignoring non-targets â€” in multiplayer, have everyone freeze at once, or make a deliberate sound and watch it beeline to that spot at increased speed. Don't do this test in a narrow corridor: it kills within 1.5m even without detecting you.",
        zeroEv: "Zero-evidence tells: hunts that ignore still, silent players entirely; the ghost visibly walking to where sounds were made rather than where players are; sudden speed-up (2.21 m/s) after any footstep, voice, or electronic noise. Hiding spots are unnecessary against a Kormos â€” standing still away from its route is safer, since it can kill through a closet wall if it wanders within 1.5m. Evidence trap: Kormos (Orbs + Spirit Box + UV) is one of three ghosts â€” with Hantu and Onryo â€” whose full evidence set does NOT rule out The Mimic (fake Orbs + Box + UV). Check for Freezing: if the room freezes, it's The Mimic.",
        tags: ['fast'],
        speedStates: [{"label": "No sound detected", "speed": 1.7}, {"label": "Moving to a sound", "speed": 2.21}]
    }
    ,
    { 
        name: "Deildegast", ev: ['emf','writing','dots'], danger: "High", hunt: "50%", speed: "3.0 â†’ 0.4 m/s", blink: "Normal", forced: null,
        traits: ["Fastest Base Speed", "Slowed By Moved Items", "No LoS Acceleration"],
        desc: "A Norwegian boundary-stone ghost that starts every hunt at the fastest base speed in the game â€” but every unique item players move before a hunt slows it, all the way down to a crawl.",
        ability: "Starts each hunt at <span class='hl-red'>3.0 m/s</span> â€” the fastest base speed of any ghost â€” with NO line-of-sight acceleration, so it never speeds up on seeing you. Every unique prop moved or interacted with by players (<span class='hl-green'>including dead players</span>) outside of hunts cuts its next hunt speed by 0.1 m/s: throwables, light switches, the fuse box, taps â€” but NOT doors, cursed possessions, or your own equipment, and each item only counts once. 13 items brings it to standard speed (1.7 m/s); 26 items hits its floor of <span class='hl-green'>0.4 m/s</span>. The counter <span class='hl-red'>resets to 3.0 m/s after every hunt</span> (and when a crucifix burns), so keep moving items between hunts. It is also shier than most: only a 10% chance to touch doors and light switches (vs the usual 25%), and it can stay quiet and hidden longer than other ghosts.",
        test: "Litter Test: Between hunts, have the team pick up or throw 10â€“15 unique house items (dead teammates can help â€” their interactions count). If the next hunt is noticeably slow, then snaps back to a flat-out sprint the hunt after you stop littering, it's a Deildegast. Reverse check: touch nothing after a hunt â€” a ghost hunting at a constant, blistering 3.0 m/s with zero line-of-sight speed-up, even while staring right at you, is near-conclusive.",
        zeroEv: "A flat 3.0 m/s with no LoS acceleration is the giveaway: a Revenant only hits 3.0 m/s AFTER detecting someone (very slow otherwise), while a fresh Deildegast is 3.0 m/s constantly. Confirm by moving unique items between hunts and listening for the next hunt to slow â€” the effect resets after every hunt and every burned crucifix, so re-litter each time. At full speed it is faster than you: rely on crucifixes, incense, and tight loops early, and keep ~15 items moved between hunts to make it manageable. If activity goes dead quiet, reposition objects around the area to provoke it.",
        tags: ['fast'],
...
// â”€â”€â”€ FOOTSTEP TEMPO ENGINE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const TempoEngine = (() => {
    let ctx = null;
    let tickTimer = null;
    ...
    function start(speed, btn) {
        ...
        if (!ms) return; // Variable speed ghost â€” nothing to play

        running = true;
        activeButton = btn;
        if (btn) {
            btn.classList.add('tempo-btn-active');
            btn.querySelector('.tempo-btn-icon').textContent = 'â– ';
        }
...
function updateBoard() {
    ...
    if (ghostSearchMeta) {
        if (app.search.ghosts) {
            ...
            ghostSearchMeta.textContent = `Showing ${matches.length} ghost${matches.length === 1 ? '' : 's'} with name matching â€œ${app.search.ghosts}â€`;
        } else {
...
function openGhostModal(g) {
    ...
    const forcedNote = g.forced ? `<div style="margin-top:8px; padding:6px 10px; background:rgba(6,182,212,0.1); border:1px solid rgba(6,182,212,0.3); border-radius:6px; font-size:0.82rem; color:var(--acc-cyan);">âš¡ <strong>Guaranteed Evidence:</strong> This ghost <em>always</em> shows <strong>${g.forced.toUpperCase()}</strong> even on Nightmare/Insanity.</div>` : '';
    ...
    const tempoButtons = states.map((s, i) => {
        ...
        return `<button class="tempo-btn${isVariable ? ' tempo-btn-variable' : ''}"
            ...
            title="${isVariable ? 'Speed varies â€” copies the ghost being mimicked' : `Tap to hear ${s.label} footstep tempo`}">
            <span class="tempo-btn-label">${s.label}</span>
            <span class="tempo-btn-speed">${s.speed} m/s</span>
            <span class="tempo-btn-bpm">${isVariable ? 'â€” BPM' : bpm + ' BPM'}</span>
            <span class="tempo-btn-icon">${isVariable ? '~' : 'â–¶'}</span>
        </button>`;
    }).join('');
    ...
    document.getElementById('mContent').innerHTML = `
        ...
        <div class="tempo-section">
            <div class="tempo-header">
                <span class="tempo-title">ðŸ‘Ÿ FOOTSTEP TEMPO</span>
        ...
        ${forcedNote}
        <div class="section-header" style="color:#a78bfa; border-color:rgba(139,92,246,0.4);">ðŸŽ² Zero Evidence Tell</div>
        ...
        <div class="section-header">ðŸ”¬ Confirmation Test</div>
...
const MANUAL_DB = {
    ev: `
        <div class="manual-entry">
            <h4>EMF Level 5</h4>
            <p class="detail-text">
                Reader must hit the red LED (5). Level 2â€“4 is normal interaction or events.
        ...
        <div class="manual-entry">
            ...
            <p class="detail-text">
                ...
                Look for the <span class="hl-green">ghost icon</span> on the box UI â€” that's evidence. <span class="hl-red">Deogen</span> is unique: responds in any lighting, no dark needed.
                <br><span class="hl-blue">Tip:</span> Ask location or aggression questions. Check the box UI carefully â€” a red LED with no ghost icon means it heard you but no response.
            </p>
        </div>
        <div class="manual-entry">
        ...
        <div class="manual-entry">
            <h4>Ghost Writing</h4>
            <p class="detail-text">
                Ghost writes in the book. A book being thrown or moved is <span class="hl-red">not</span> evidence â€” only actual writing counts.
        ...
        <div class="manual-entry">
            ...
            <p class="detail-text">
                Must read <span class="hl-red">below 0Â°C/32Â°F</span> on a thermometer. <span class="hl-red">Visible breath is NOT evidence</span> â€” since v0.9.0, player breath appears below 5Â°C regardless of ghost type. Do not rely on breath alone.
    ...
    sanity: `
        <div class="manual-entry">
            ...
            <p class="detail-text">
                ...
                <br>&bull; In darkness, drain is roughly <span class="hl-red">0.09â€“0.12%/sec on small maps</span> (lower on bigger maps), then multiplied by difficulty. Playing solo halves it.
        ...
        <div class="manual-entry">
            ...
            <p class="detail-text">
                ...
                <br>&bull; One low-sanity player can drag the whole teamâ€™s average down.
                <br>&bull; Exception: <span class="hl-red">Banshee</span> hunts based on its chosen <em>target's</em> individual sanity â€” not the team average. Keep the target's sanity high to suppress hunts.
            </p>
        </div>
        <div class="manual-entry">
    ...
    cursed: `
        <div class="manual-entry">
            ...
            <p class="detail-text">
                ...
                <br><span class="hl-blue">Tip:</span> Use deliberately when near a hiding spot â€” great for forcing ghost events and gathering evidence quickly.
            </p>
        </div>
        <div class="manual-entry">
            <h4>Tarot Cards</h4>
        ...
        <div class="manual-entry">
            <h4>Music Box</h4>
            <p class="detail-text">
                Plays a lullaby, revealing the ghostâ€™s location.
                ...
                <br>&bull; <span class="hl-red">Do NOT throw it</span> â€” throwing (slamming shut) immediately starts a cursed hunt. Always place it gently.
        ...
        <div class="manual-entry">
            ...
            <p class="detail-text">
                ...
                <br>&bull; Light all 5 candles to summon the ghost â€” it teleports to the circle and manifests. After the event there is a ~5 second grace period before the cursed hunt begins.
    ...
    equip: `
        ...
        <div class="manual-entry">
            ...
            <p class="detail-text">
                ...
                <br><span class="hl-blue">Tip:</span> Save pills until after youâ€™ve gathered some evidence, then stabilize before pushing late game.
            </p>
        </div>
        <div class="manual-entry">
            <h4>Candles & Lighters</h4>
            <p class="detail-text">
                Firelights slow passive sanity drain while nearby (exact amount depends on Firelight tier â€” they reduce it, but may not stop it entirely at all tiers).
                <br>&bull; The ghost can blow them out.
                <br>&bull; <span class="hl-red">Onryo:</span> Flames act as crucifixes â€” blocks hunt attempts within 4m. After the 3rd flame blow-out, Onryo can hunt at any sanity.
        ...
        <div class="manual-entry">
            ...
            <p class="detail-text">
                ...
                <br><span class="hl-blue">Tip:</span> Try multiple angles if you suspect Orbs but havenâ€™t seen any yet.
            </p>
        </div>
        <div class="manual-entry">
            <h4>Photo Camera</h4>
...
const EQUIPMENT = {
    detection: [
        ...
        { name: "Thermometer", tier: "Starter", cost: "$30", usage: "Find cold rooms. Freezing Temps = below 0Â°C/32Â°F evidence.", mechanics: "Rooms cool over time. Breaker OFF = faster cooling. Yellow/Orange = room temp, Blue/Purple = active area", tips: "Sweep building early. Ghost room always coldest. Helps locate ghost quickly.", range: "Point and shoot - instant reading" },
    ...
    protection: [
        { name: "Crucifix", tier: "Essential", cost: "$30", usage: "Prevents hunts from starting within range. T1 blocks 1 hunt; T2/T3 block 2.", mechanics: "Range by tier: T1 3m / T2 4m / T3 5m (+50% vs Demon: 4.5/6/7.5m). Works held, thrown, or placed. After a block the ghost waits 25s (20s Demon) before trying again. T3 with both charges left can block a cursed hunt (uses both charges).", tips: "Placing it on the floor in the centre of the active area frees your hands â€” but it does work while held. Both the ghost's feet AND head (1.5m up) must be in range, so T1 range is effectively tight. Range ignores walls.", range: "T1 3m / T2 4m / T3 5m (+50% vs Demon)", uses: "T1: 1 Â· T2/T3: 2" },
...
window.showEquipTab = function(category) {
    ...
    if (meta) {
        if (query) {
            ...
            meta.textContent = `Showing ${itemsWithCategory.length} equipment item${itemsWithCategory.length === 1 ? '' : 's'} with name matching â€œ${query}â€ across all tabs`;
        } else {
...
function initGoogleAuth() {
    ...
    document.getElementById('btnResetStats').addEventListener('click', async () => {
        ...
        const confirmed = confirm('âš ï¸ Are you sure you want to reset ALL your stats?\n\nThis will delete:\n- All investigation history\n- Win/loss records\n- XP and level\n- Everything!\n\nThis CANNOT be undone!');
        ...
        if (finalConfirm === 'YES') {
            try {
                ...
                alert('âœ… Stats reset successfully!');
                ...
                alert('âŒ Failed to reset stats. Please try again.');
            }
        } else {
...
async function handleGoogleLogin() {
    if (!auth) {
        alert("âš ï¸ Firebase Auth not ready yet.\n\nPlease wait a moment and try again.");
                ...
                historyItem.innerHTML = `
                    <div class="history-info">
                        <div class="history-guess">
                            ${item.correct ? 'âœ…' : 'âŒ'} ${item.actualGhost || item.actual}
...
function showInvestigationBanner() {
    ...
    banner.innerHTML = `
        <div class="investigation-info">
            ðŸ” Investigation in progress (${currentInvestigation.mode.toUpperCase()} EVI)
...
async function submitActualGhost(actualGhost) {
    ...
    try {
        ...
        resultMessage += `Your stats:\nðŸ† ${newStats.wins} wins out of ${newStats.total} total\nðŸ“Š ${winRate}% accuracy\nâ­ Level ${newStats.level} (${newStats.xp} XP)`;
...
updateUserCount = function() {
    ...
    usersRef.once('value', (snapshot) => {
        ...
        if (!users) {
            ...
            if (usersList) {
                usersList.innerHTML = '<div class="user-chip you">ðŸ‘¤ You</div>';
            }
            return;
        }
        ...
        if (usersList) {
            ...
            activeUsers.forEach(user => {
                ...
                if (user.photoURL) {
                    ...
                    chip.innerHTML = `
                        <span>ðŸ‘¤ ${user.nickname}${user.isYou ? ' (You)' : ''}</span>
                    `;
                }
...
function renderFriendRequests() {
    ...
    pendingRequests.incoming.forEach(([uid, request]) => {
        ...
        item.innerHTML = `
            ...
            <button class="btn-request-accept" type="button">âœ“</button>
            <button class="btn-request-decline" type="button">âœ—</button>
        `;
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('ðŸš€ Initializing new top bar...');
    ...
    setInterval(function() {
        ...
        if (isLoggedIn) {
            ...
            if (avatar && nickname && level) {
                ...
                console.log('ðŸ“¸ Avatar URL:', avatarSrc);
                ...
                if (userState !== lastUserState) {
                    ...
                    const avatarHTML = avatarSrc && avatarSrc !== '' 
                        ...
                        : `<div style="width: 22px; height: 22px; border-radius: 50%; border: 2px solid var(--acc-cyan); background: var(--acc-cyan); display: flex; align-items: center; justify-content: center; font-size: 0.8rem;">ðŸ‘¤</div>`;
        ...
        sections: [
            {
                ...
                items: [
                    ...
                    "Kormos and Aswang added as surprise new ghosts. (Site note: their data has since been verified against wiki datamines â€” see the ghost cards.)"
                ]
            }
        ]
    }
...
async function loadGameUpdates() {
    try {
        ...
        // Offline / file:// usage â€” the baked-in GAME_UPDATES covers us.
    }
    renderGameUpdates();
}
    ...
    (GAME_UPDATES_LIVE || GAME_UPDATES).forEach((update, index) => {
        ...
        card.innerHTML = `
            ...
            <div class="update-body">
                ${sectionsHtml}
                ${update.url ? `<p class="update-source-link"><a href="${escapeUpdateHtml(update.url)}" target="_blank" rel="noopener noreferrer">View full notes on Steam â†—</a></p>` : ""}
            </div>
        `;
...
window.openNewUserMenu = function() {
    console.log('ðŸ”µ New user menu button clicked!');
    const dropdown = document.getElementById('userDropdown');
    if (dropdown) {
        console.log('ðŸ”µ Toggling dropdown, current display:', dropdown.style.display);
        dropdown.style.display = dropdown.style.display === 'none' ? 'block' : 'none';
        console.log('ðŸ”µ New display:', dropdown.style.display);
    } else {
        console.error('âŒ User dropdown not found!');
    }
};
